...
import type { ServiceProvider, ServiceOffering, CompanyCertification, PersonnelQualification } from '../../lib/types';
...
export function ProviderCard({ provider }: ProviderCardProps) {
  ...
  // Placeholder for user ID, replace with your auth logic
  ...
  const handleBookNow = async () => {
    ...
    try {
      ...
      if (!serviceId) throw new Error('No service available for this provider.');
      const res = await fetch('/api/payment/checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ providerServiceId: serviceId, buyerId: userId })
      });
  ...
  return (
    <Card className="overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full">
      ...
      <CardContent className="flex-grow space-y-3">
        ...
        {provider.services && provider.services.length > 0 && (
            <div><h4 className="text-sm font-semibold mb-1 flex items-center"><CheckSquare className="h-4 w-4 mr-1 text-primary"/>Key Services:</h4>
                <div className="flex flex-wrap gap-1">
                    {provider.services.slice(0, 3).map((service: ServiceOffering) => (<Badge key={service.id || service.name} variant="outline" className="text-xs">{service.name}</Badge>))}
        ...
        {provider.certifications && provider.certifications.length > 0 && provider.isCompany && (
           <div><h4 className="text-sm font-semibold mb-1 flex items-center"><Award className="h-4 w-4 mr-1 text-primary"/>Company Certs:</h4>
            <div className="flex flex-wrap gap-1">
              {provider.certifications.slice(0, 2).map((cert: CompanyCertification) => (<Badge key={cert.id} variant="outline" className="text-xs bg-blue-50 border-blue-200 text-blue-700">{cert.name}</Badge>))}
        ...
        {provider.personnelQualifications && provider.personnelQualifications.length > 0 && (
           <div><h4 className="text-sm font-semibold mb-1 flex items-center"><Users2 className="h-4 w-4 mr-1 text-primary"/>Personnel Certs:</h4>
            <div className="flex flex-wrap gap-1">
              {provider.personnelQualifications.slice(0, 2).map((qual: PersonnelQualification) => (
                <Badge key={qual.id} variant="outline" className="text-xs bg-indigo-50 border-indigo-200 text-indigo-700">
                  {Number(qual.quantity) > 1 ? `${qual.quantity}x ` : ''}{qual.certificationBody} {qual.level}
                </Badge>
              ))}
        ...
        {provider.availableDocuments && provider.availableDocuments.length > 0 && (
           <div><h4 className="text-sm font-semibold mb-1 flex items-center"><BookOpen className="h-4 w-4 mr-1 text-primary"/>Docs Available:</h4>
            <div className="flex flex-wrap gap-1">
              {provider.availableDocuments.slice(0, 2).map((doc: string) => (<Badge key={doc} variant="outline" className="text-xs bg-teal-50 border-teal-200 text-teal-700">{doc}</Badge>))}
      ...
      <CardFooter className="flex flex-col sm:flex-row sm:justify-end items-stretch sm:items-center gap-2 pt-4 border-t">
        ...
        <Button size="sm" className="w-full sm:w-auto bg-primary text-white" onClick={handleBookNow} disabled={isBooking}>
